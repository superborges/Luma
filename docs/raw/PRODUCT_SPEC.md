...
> **文档状态（必读）**  
> 本文档描述 **长期愿景与完整技术方案**（**raw**），篇幅与能力范围 **大于** 当前主应用已实现部分。  
> **与 `main` / `v1` 代码行为一致、用于迭代对齐的基线** 请以 **`docs/MVP/Build Spec.md`** 与 `docs/MVP/*.md` 为准。  
> 若本文件与实现冲突：**以实现与 MVP 文档为准**，并应更新本文件时标注为「未来 / v2+」或修订章节。
