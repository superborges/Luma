...
> **文档状态（2026-05-05 更新）**  
> 本文档描述的完整产品规格已于 **V3（2026-05）全部实现**。  
> 五阶段核心管线（Ingest → Group → Score → Cull → Export/Archive）及其所有子功能均已落地并通过验收。  
>
> **未纳入实现的边缘项**（评估后决定不做）：  
> - SSE 流式响应（整批 await 已满足需求）  
> - 自定义 Prompt 模板（两套内置 Prompt 足够）  
> - Shortcuts / 自动化（AppIntents，当前无用户场景）  
> - Lightroom CC 专属适配（CC 可手动导入，Classic 自动导入已覆盖主流程）  
> - 本地大模型特殊优化（Ollama 走 `.openAICompatible` 通道即可）  
>
> 如需了解实现细节与版本演进，参见 `docs/V1/`、`docs/V2/`、`docs/V3/` 下的 Build Spec。
